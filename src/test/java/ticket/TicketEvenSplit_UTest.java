...
public class TicketEvenSplit_UTest {
    ...
    public void t_whoPaid() {
        ...
        Assert.assertEquals("Testing user - input=Gones Anseel id 1", gones, airplaneEvenSplitTicket.whoPaid());
    }
    ...
    public void t_setInitialBalances() {
        ...
        Assert.assertEquals("balance Gones = +50EU", 50.0, airplaneEvenSplitTicket.getBalances().get(gones), .1);
        Assert.assertEquals("balance Matthias = -50EU", -50.0, airplaneEvenSplitTicket.getBalances().get(matthias), .1);
        ...
        Assert.assertFalse("ticketBalances.find(stijn) = false", airplaneEvenSplitTicket.getBalances().containsKey(stijn));
    ...
    public void t_addDebtor() {
        ...
        User stijn = new User("Stijn", "VR", 3);
        airplaneEvenSplitTicket.addDebtor(stijn);

        Assert.assertTrue("ticketBalances.find(stijn) = false", airplaneEvenSplitTicket.getBalances().containsKey(stijn));

        Assert.assertEquals("balance Gones = +66.67EU", 66.67, airplaneEvenSplitTicket.getBalances().get(gones), .1);
        Assert.assertEquals("balance Matthias = -33.33EU", -33.33, airplaneEvenSplitTicket.getBalances().get(matthias), .1);
        Assert.assertEquals("balance Stijn = -33.33EU", -33.33, airplaneEvenSplitTicket.getBalances().get(stijn), .1);
    ...
    public void t_getDebtors() {
        ...
        Set<User> assumedDebtors = airplaneEvenSplitTicket.getDebtors();
    ...
    public void t_addPayment() {
        ...
        airplaneEvenSplitTicket.addPayment(stijn, 33.33);
        Assert.assertEquals("balance stijn = 0.0", 0.0, airplaneEvenSplitTicket.getBalances().get(stijn), .1);
        Assert.assertEquals("balance Matthias = -33.33EU", -33.33, airplaneEvenSplitTicket.getBalances().get(matthias), .1);
        Assert.assertEquals("balance Gones = +33.33EU", 33.33, airplaneEvenSplitTicket.getBalances().get(gones), .1);

        airplaneEvenSplitTicket.addPayment(matthias, 33.33);
        Assert.assertEquals("balance stijn = 0.0", 0.0, airplaneEvenSplitTicket.getBalances().get(stijn), .1);
        Assert.assertEquals("balance Matthias = 0", 0.0, airplaneEvenSplitTicket.getBalances().get(matthias), .1);
        Assert.assertEquals("balance Gones = 0", 0.0, airplaneEvenSplitTicket.getBalances().get(gones), .1);
